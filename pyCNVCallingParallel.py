...
def execScript(sample):
    user=getpass.getuser()
    script2fixNA="/home/j/jacquese/"+user+"/scratch/BIGFILE/UKBB_CNV/AnalysisScripts_CNVcalling/NAtoNaNscript.sh"
    subprocess.call([script2fixNA,sample])
    hmmFile=sys.argv[12]
    batch=sys.argv[6]
    ...
    if (os.path.isfile(outputQuantiSNP+"/"+sampleID+".outdir/"+sampleID+".cnv")==True and os.path.isfile(outputQuantiSNP+"/"+sampleID+".outdir/"+sampleID+".qc")==True) or (os.path.isfile(outputPennCNV+"/CNV_DATA/autosome_"+sampleID+".rawcnv")==True and os.path.isfile(outputPennCNV+"/CNV_DATA/gonosome_X_"+sampleID+".rawcnv")==True and os.path.isfile(outputPennCNV+"/CNV_DATA/autosome_"+sampleID+".log")==True and os.path.isfile(outputPennCNV+"/CNV_DATA/gonosome_X_"+sampleID+".log")==True):
        reformat=False
    else:
        reformat=True
    if sys.argv[14]=="True":
        if (os.path.isfile(outputQuantiSNP+"/"+sampleID+".outdir/"+sampleID+".cnv")==False or os.path.isfile(outputQuantiSNP+"/"+sampleID+".outdir/"+sampleID+".qc")==False):
            if reformat == True:
                ...
                print("Sample NA entries already fixed")
